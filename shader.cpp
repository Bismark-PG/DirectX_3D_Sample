...
bool Shader_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	// Filtering
	// LINEAR	>>  無理に拡張させるので解像度が低下
	//				基本的に高い解像度のテクスチャを縮小処理
	// POINT	>>  鮮明になる (代表点が出る)
	sampler_desc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;

	// UV参照外の取り扱い (UV Addressing Mode)
	// コピーする容量はUVによって決める
	//		U : Height || V : Width
	// Clamp Mode	 = 下の最終ピクセルをコピー (Copy End Pixel)
	// Warp Mode	 = 同じ絵をUVによってコピー　(Copy Same Texture)
	// Mirror Mode	 = 相対方向
	// Border Mode	 = UVの範囲の中から絵の以外の空間に色を入れる
	//					>> If use this mode need "BorderColor"
...
void Shader_SetProjectionMatrix(const XMMATRIX& matrix)
{
	// 定数バッファ格納用行列の構造体を定義
...
void Shader_SetWorldMatrix(const XMMATRIX& matrix)
{
	// 定数バッファ格納用行列の構造体を定義
	XMFLOAT4X4 transpose;
